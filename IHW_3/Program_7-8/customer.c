...
int main() {
    ...
    for (int i = 0; i < list_length && !shutdown_flag; i++) {
        ...
        if (department == 1) {
            ...
            // Уведомляем продавца_1 о новом покупателе
        ...
        int served = 0; // Флаг успешного обслуживания
        while (!served && !shutdown_flag) {
            struct timespec ts;
            clock_gettime(CLOCK_REALTIME, &ts); // Текущее время
            ts.tv_sec += 2; // Таймаут 2 секунды
            
            int wait_result = sem_timedwait(my_sem, &ts); // Ожидаем покупку
            if (wait_result == 0) { // Если покупка получена
                served = 1; // Ура, нас обслужили
                printf("Покупатель %d: Купил товар из отдела %d\n", customer_id, department);
            } else if (errno != ETIMEDOUT) { // Если время истекло
