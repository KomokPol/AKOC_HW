...
int observer_fd = -1; // Дескриптор FIFO
...
int init_shared_resources(void) {
    int created = 0; // Флаг создания разделяемой памяти
    ...
    if (created) { // Если память только что создана
        ...
        // Если не создан, убедимся, что размер корректен
        if (ftruncate(shm_fd, sizeof(shop_t)) == -1 && errno != EINVAL) {
            // обычно фtruncate на уже заданный размер выдаст 0 или EINVAL; можно игнорировать
        }
    }
...
void send_to_observer(const char *message, const char *process_type, int pid) {
    if (observer_fd == -1) { // Если FIFO не открыт
        return;
    }
    ...
    char timestamp[20];
    // Форматируем временную метку в формате ЧЧ:ММ:СС
    strftime(timestamp, sizeof(timestamp), "%H:%M:%S", tm_info);
    // Формируем полное сообщение с временем, типом процесса, PID и текстом
    snprintf(buffer, sizeof(buffer), "[%s] %s (PID: %d): %s\n", timestamp, process_type, pid, message);
    // Отправляем сообщение в FIFO
    write(observer_fd, buffer, strlen(buffer));
}

// Инициализация FIFO для наблюдателя
void init_observer_fifo(void) { 
    if (mkfifo(OBSERVER_FIFO, 0666) == -1) { // Если не удалось создать FIFO
        if (errno != EEXIST) { // Если не существует
            perror("mkfifo observer");
        }
    }
    
    // Открываем для записи (неблокирующе)
    observer_fd = open(OBSERVER_FIFO, O_WRONLY | O_NONBLOCK);
    if (observer_fd == -1) {
...
void cleanup_resources(void) {
    ...
    close_observer_fifo(); // Закрываем FIFO для наблюдателя
}